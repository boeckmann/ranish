   ...
   while ( argc != 0 && ( argv[0][0] == '-' || argv[0][0] == '/' ) ) {
      if ( argv[0][1] == 'D' || argv[0][1] == 'd' && argc > 1 ) {
         ...
         if ( sscanf( argv[1], "%u,%u,%u", &cyls, &heads, &sects ) != 3 ||
              heads > 255 || sects > 63 ) {
            fprintf( stderr, "Invalid CHS geometry given.\n" );
            exit( 1 );
         }
...
void start_gui( void )
{
   ...
   while ( 1 ) {
      ...
      if ( reported_total_sectors != dinfo.total_sects ) {
         show_error(
            "Can not access all sectors BIOS reports. Disk size adjusted!" );
      }
...
int setup_mbr( struct part_long *p )
{
   ...
   static struct field_desc {
      ...
      int pos;
      int len;
   } *fd, fd_chs[8] = { { 0, 4, 8 },    { 0, 13, 23 }, { 8555167, 34, 7 },
                        { 255, 42, 3 }, { 63, 46, 3 }, { 8555167, 50, 7 },
                        { 255, 58, 3 }, { 63, 62, 3 } },
          fd_lba[4] = { { 0, 4, 8 },
                        { 0, 13, 23 },
                        { 4294967295, 34, 10 },
   ...
   if ( p->level == 0 &&
      ...
      if ( disk_read_rel( p, mbr->x.adv.rel_sect, adv, ADV_DATA_SECT ) ==
           -1 ) {
   ...
   if ( view == VIEW_ADV ) {
      ipl_chksum = calc_chksum( (unsigned short *)mbr->x.adv.code,
                                sizeof( mbr->x.adv.code ) / 2 );
   ...
   if ( p->inh_invalid || p->inh_changed ) {
      mesg = TEXT(
         "One or more parent MBR not saved or invalid! Changes can not be saved." );
   }
   ...
   while ( 1 ) {
      ...
      {
         if ( view == VIEW_ADV ) {
            write_string( ACTIVE_COLOR, 5, row - top + 9,
      ...
      {
         write_string( ACTIVE_COLOR, 11, row - top + 9,
      ...
      if ( field > 1 ) {
         ...
         write_int( ACTIVE_COLOR, fd->pos, row - top + 9, fd->len,
                    *edit_target );
         move_cursor( fd->pos + fd->len - 1, row - top + 9 );

         if ( mode == MODE_CHS ) {
            sprintf( tmp, HINT_CHS,
                     ext_fix ? p->end_cyl + 1
                             : part[row].container->start_cyl,
                     part[row].container->end_cyl, dinfo.num_heads - 1,
                     dinfo.num_sects );
         }
         else {
            sprintf( tmp, HINT_LBA, ext_fix ? p->rel_sect + p->num_sect : 1,
...
#define NUM_ARGS 10
         ...
         if ( QUICK_BASE( p ) > QUICK_BASE( p ) + p->num_sect ) {
            show_error( TEXT(
               "^Partition crosses 2TiB boundary. Refusing to format!" ) );
            continue;
         }

         /* make test read to check if whole partition is accessible */
         if ( disk_read_rel( p, p->num_sect - 1, scratch_sector, 1 ) ==
              FAILED ) {
            show_error( TEXT(
               "^Can not access last sector of partition. Refusing to format!" ) );
            continue;
         }
         ...
         if ( part_is_fat_type( &part[row] ) ) {
            ...
            format_type = two_btn_dialog(
               "Shall I do a normal format or overwrite data with zeros (slow)?",
            ...
            if ( enter_string( 4, 10, TEXT( "Volume name:" ), sizeof( tmp ),
                               tmp, tmp2 ) == 0 ) {
               mesg = WARN_FORMAT_CANCEL;
               continue;
            }
            ...
            if ( enter_string( 4, hint_y, PROMPT_FORMAT, sizeof( msg_buf ),
                               msg_buf, tmp2 ) == 0 ) {
               continue;
            }
         }
         ...
         fstatus =
            os_desc[part[row].os_num].format( &part[row], argv, &format_msg );
         ...
         if ( fstatus == OK ) {
            ...
            snprintf( msg_buf, sizeof( msg_buf ),
                      TEXT( "Format completed with warnings: %s" ),
                      format_msg );
            mesg = msg_buf;
         }
            ...
            if ( format_msg ) {
               snprintf( msg_buf, sizeof( msg_buf ),
                         TEXT( "Format error: %s" ), format_msg );
            }
            else {
               snprintf( msg_buf, sizeof( msg_buf ),
                         TEXT( "Format failed!" ) );
           ...
           CLICK( 21, 11, 32 ) ) /* Verify */
      {
         ...
         if ( fstatus == OK ) {
            ...
            sprintf( tmp, "BIOS error code %02X",
                     (unsigned int)diskio_errno );
            show_error( tmp );
         }
      ...
      if ( ev.key == 'e' || ev.key == 'E' || CLICK( 22, 11, 33 ) ) {
         ...
         if (
            !two_btn_dialog(
               "This will overwrite the whole parition with zeros. All data is lost!",
               "proceed", 1, "cancel", 0 ) ) {
      ...
      if ( ev.scan == 0x52E0 || ev.scan == 0x5200 || ev.key == 'T' ||
           ev.key == 't' ) /* Insert */
      {
         ...
         if ( part[row].empty && part[row].os_num != 0 ) {
            if ( mode == MODE_CHS ) {
               ...
               if ( ABS_REL_SECT( &part[row] ) <=
                    ABS_REL_SECT( part[row].container ) ) {
      ...
      if ( ev.scan == 0x53E0 || ev.scan == 0x5300 ) /* Delete */
      {
         ...
         if ( part[row].os_id == 0x0500 || part[row].os_id == 0x0F00 ||
            ...
            if ( !two_btn_dialog(
                    "Delete this extended and all nested partitions?", "yes",
                    1, "no", 0 ) ) {
      ...
      if ( ev.scan == 0x3C00 || CLICK( 24, 15, 31 ) ) /* F2 - Save */
      {
         ...
         if ( view == VIEW_ADV &&
              ( disk_write_rel( p, mbr->x.adv.rel_sect, adv,
                                ADV_DATA_SECT ) == -1 ||
      ...
      if ( view != VIEW_ADV && ( ev.key == 'H' || ev.key == 'h' ||
                                 CLICK( 19, 11, 38 ) ) ) { /* Hide */
      ...
      if ( view == VIEW_EMBR &&
         ...
         mesg = MESG_INSTALL;
                ...
                ( ev.key == 'R' || ev.key == 'r' ||
                  CLICK( 17, 11,
                         60 ) ) ) { /* Read how to make ext bootable */
         ...
         force_redraw_menu = 1;
                ...
                CLICK( 20, 42, 64 ) ) /* Save */
      {
         if ( enter_string( 20, 10, PROMPT_FILE, sizeof( file_name ),
                            file_name, 0 ) == 0 ) {
            continue;
         }
         if ( view != VIEW_ADV &&
                ...
                CLICK( 21, 42, 66 ) ) /* Load */
      {
         ...
         if ( view != VIEW_ADV &&
            ...
            if ( view == VIEW_ADV && strcmp( zz, ADV_DATA_SIGNATURE ) != 0 ) {
               ...
               mesg =
                  ( view == VIEW_ADV ) ? MESG_ADVFILE_LOADD : MESG_FILE_LOADD;
               if ( view == VIEW_MBR &&
...
void command_line( int argc, char **argv )
{
   ...
   if ( ( p = malloc( sizeof( struct part_long ) *
                      ( MAX_PART_ROWS + 1 ) ) ) == 0 ) {
      cmd_error( ERROR_MALLOC );
      free( data );
      return;
   ...
   if ( argv[0][0] == 'P' || argv[0][0] == 'p' ) /* Print partition table */
   {
      ...
      printf( HEADER_CMD, dinfo.disk - 127, dinfo.total_sects / 2048,
         ...
         for ( i = 0; i < num_rows; i++ ) {
            if ( !part[i].empty && part[i].valid && part[i].os_id != 0 &&
               ...
               printf(
                  ...
                  "]\n\n",
                  sprintf_os_name( tmp, &part[i] ), part[i].num_sect / 2048,
                  sprintf_long( tmp2, part[i].num_sect ), part[i].start_cyl,
                  part[i].start_head, part[i].start_sect );
   ...
   if ( argv[0][0] == 'A' || argv[0][0] == 'a' ) /* Activate */
   {
      ...
      if ( view == VIEW_ADV ) {
         printf(
            "Command has no effect, because Advanced Boot Manager is installed.\n" );
         goto done;
      }
      if ( argc < 2 ) {
         usage();
   ...
   if ( argv[0][0] == 'F' || argv[0][0] == 'f' ) /* Format */
   {
      ...
      if ( QUICK_BASE( p ) > QUICK_BASE( p ) + p->num_sect ) {
         cmd_error(
            TEXT( "^Partition crosses 2TiB boundary. Refusing to format!" ) );
      }
