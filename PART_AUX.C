...
void adjust_part_type( struct part_long *p, int interactive )
{
   ...
   if ( QUICK_END( p ) >= 1024ul * 255ul * 63ul && p->level == 1 ) {
      ...
      if ( p->os_id == 0x0500 || p->os_id == 0x1500 ) {
         if ( !interactive ||
              two_btn_dialog( "Partition type should be Extend LBA", "change",
                              1, "ignore", 0 ) ) {
      ...
      if ( p->os_id == 0x0B00 || p->os_id == 0x1B00 ) {
         if ( !interactive ||
              two_btn_dialog( "Partition type should be FAT-32 LBA", "change",
                              1, "ignore", 0 ) ) {
   ...
   if ( QUICK_END( p ) < 1024ul * 255ul * 63ul || p->level > 1 ) {
      if ( p->os_id == 0x0F00 || p->os_id == 0x1F00 ) {
         if ( !interactive ||
              two_btn_dialog( "Partition type should be Extend non-LBA",
                              "change", 1, "ignore", 0 ) ) {
            part_set_id( p, p->os_id - 0x0A00 );
         }
      }
      if ( p->os_id == 0x0C00 || p->os_id == 0x1C00 ) {
         if ( !interactive ||
              two_btn_dialog( "Partition type should be FAT-32 non-LBA",
                              "change", 1, "ignore", 0 ) ) {
            part_set_id( p, p->os_id - 0x0100 );
         }
      }
                         ...
                         int ( *bbt_func )( unsigned long ) )
{
   ...
   return disk_process_sectors( disk_verify, daddr, num_sect, 0, func,
                                bbt_func );
}
                       ...
                       int ( *bbt_func )( unsigned long ) )
{
   ...
   return disk_process_sectors( disk_write, daddr, num_sect, fill_value, func,
                                bbt_func );
}
...
void pack_part_tab( struct part_long *part, struct part_rec *part_rec, int n )
{
   ...
   for ( i = 0; i < n; i++ ) {
      ...
      if ( ( num_sect > 0 ) && ( part[i].start_cyl > 1023 ) ) {
         ...
         part_rec[i].start_sectCylH = ( part[i].start_sect & 0x3f ) |
                                      ( ( part[i].start_cyl >> 2 ) & 0xc0 );
...
static int is_lba_chs_marker( unsigned long cyl, unsigned int head,
                              unsigned int sect )
{
   return ( cyl == 1023 ) && ( head >= 254 ) && ( sect == 63 );
}

static int truncated_chs_equals_lba( unsigned long cyl, unsigned int head,
                                     unsigned int sect,
                                     unsigned long lba_sect )
{
   return ( ( sect == SECT( lba_sect ) ) && ( head == HEAD( lba_sect ) ) &&
            ( cyl == ( CYL( lba_sect ) % 1024 ) ) && CYL( lba_sect ) > 1023 );
   ;
}
   ...
   for ( i = 0; i < n; i++ ) {
      ...
      if ( part[i].rel_sect != 0 || part[i].num_sect != 0 ) {
         ...
         start_chs_marker =
            ...
            is_lba_chs_marker( part[i].start_cyl, part[i].start_head,
                               part[i].start_sect );

         end_chs_marker =
            end_sect != ABS_REL_SECT( &part[i] ) &&
            is_lba_chs_marker( part[i].end_cyl, part[i].end_head,
                               part[i].end_sect );

         start_cyl_wrapped =
            truncated_chs_equals_lba( part[i].start_cyl, part[i].start_head,
                                      part[i].start_sect, start_sect );

         end_cyl_wrapped = truncated_chs_equals_lba(
            part[i].end_cyl, part[i].end_head, part[i].end_sect, end_sect );

         if ( start_chs_marker || start_cyl_wrapped || end_chs_marker ||
              end_cyl_wrapped ) {
                    ...
                    struct part_long *container2 )
{
   ...
   for ( p = part, i = 0; i < n; i++, p++ ) {
      ...
      if ( p->mbr_err || p->range_err || p->overlap_err ||
           p->inconsist_err ) {
         p->valid = 0;
      }
   } /* p */
   ...
   for ( i = 0; i < n; i++ ) {
      if ( container2->level > 1 && part[i].os_id == OS_EXT &&
           ( QUICK_BASE( &part[i] ) <
             QUICK_BASE( container2 ) + QUICK_SIZE( container2 ) ) ) {
...
char *sprintf_partrec( char *tmp, struct part_long *p, int num, int view )
{
   ...
   if ( mode == MODE_CHS ) {
      ...
      sprintf( tmp, "%2d %-4s %s%7lu %3lu %3lu %7lu %3lu %3lu %12s ", num,
               p->active ? ( view == VIEW_ADV ? "Menu" : " Yes" ) : " No",
               sprintf_os_name( tmp1, p ), p->start_cyl, p->start_head,
               p->start_sect, p->end_cyl, p->end_head, p->end_sect, tmp3 );
   }
   else /* MODE_LBA */
   {
      tmp3 = format_size( p->num_sect, tmp2 );

      sprintf( tmp, "%2d %-4s %s%10lu %10lu %10lu%12s ", num,
               p->active ? ( view == VIEW_ADV ? "Menu" : " Yes" ) : " No",
               sprintf_os_name( tmp1, p ), p->rel_sect, p->num_sect,
               ( p->num_sect == 0 ) ? ( 0 )
                              ...
                              struct adv *adv )
{
   ...
   chksum = calc_chksum( (unsigned short *)mbr->x.adv.code,
   ...
   if ( strncmp( adv->adv_title, MANAGER_TITLE,
                 sizeof( adv->adv_title ) - 8 ) == 0 ) {
...
void print_part_details( struct part_long *part, int num_rows )
{
   ...
   for ( i = 0; i < num_rows; i++ ) {
      ...
      printf( "%s %2d %2X  %02X %4lu  %3lu  %2lu %4lu  %3lu  %2lu %8lu %8lu "
              ...
              num_rows == 4 ? 0x80 : part[i].orig_row, part[i].os_id >> 8,
