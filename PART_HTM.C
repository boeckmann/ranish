...
void html_view(char *target_name)
{
    ...
    *p = 0;
    ...
    if ((bs = strstr(text, "<BODY>")) == 0 ||
        (bq = strstr(bs += 6, "</BODY>")) == 0) {
        bs = text;
        bq = bs + strlen(text);
    }
    ...
    while (1) {
        ...
        if (bar_hooked) {
            ...
            if (forced_barpos == 0) {
                top    = bs;
                barpos = 0;
            } else if (forced_barpos == BARLEN - 1 &&
                       barpos != forced_barpos) {
                old_barpos = barpos;
                seek_base  = bq;
                seek_cnt   = -N;
                continue;
        ...
        if (force_redraw == 1 || check_mouse == 1) {
            ...
            while (1) {
                ...
                if (*p == '<' && (p[1] >= 'A' && p[1] <= 'Z' || p[1] == '/' ||
                                  p[1] >= 'a' && p[1] <= 'z' || p[1] == '!')) {
                    if (link_started == 0 &&
                        _strnicmp(p, "<A HREF=", 8) == 0) {
                        link_reference = p + 8;
                        link_started   = 1;
                    } else if (link_started == 1 &&
                               _strnicmp(p, "</A>", 4) == 0) {
                        link_started = 0;
                ...
                if (*p == '&') {
                    if (_strnicmp(p, "&gt;", 4) == 0 && bq - p > 4) {
                        ch = '>';
                        p += 3;
                    } else if (_strnicmp(p, "&lt;", 4) == 0 && bq - p > 4) {
                        ch = '<';
                        p += 3;
                    } else if (_strnicmp(p, "&quot;", 6) == 0 && bq - p > 6) {
                        ch = '\"';
                        p += 5;
                    } else if (_strnicmp(p, "&nbsp;", 6) == 0 && bq - p > 6) {
                        ch = ' ';
                ...
                if (check_mouse && line == mline && len == mpos &&
                    link_started)
            ...
            if (force_redraw == 1) {
                ...
                write_string(
                    BORDER_COLOR, X + 2 + LEN - 6, Y + 1, "\x1B Back");

                for (i = 0; i < BARLEN; i++) {
                    write_char(BORDER_COLOR,
                               BARX,
                               BARY + i,
                               (i != barpos) ? '°' : '²');
                }
        ...
        if (ev.ev_type & (EV_MOUSE | EV_TIMER)) {
            if (((ev.ev_type & EV_TIMER) || left_was_pressed == 0) &&
                ev.left == 1) {
                if (ev.x > X + 2 + LEN - 7 && ev.x < X + 2 + LEN &&
                    ev.y == Y + 1) {
                    if (num_backs == 0)
                        break;
            ...
            if (ev.x > X + 1 && ev.x < X + LEN + 2 && ev.y > Y &&
                ev.y < Y + N + 1) {
                check_mouse = 1;
                mline       = ev.y - Y - 1;
                mpos        = ev.x - X - 2;
