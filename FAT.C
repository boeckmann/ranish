...
static unsigned short fat12_cluster_size( unsigned long sectors )
{
   ...
   else if ( sectors <= 4123 ) {
      return 1;
   }
   else if ( sectors <= 8207 ) {
      return 2;
   }
   else if ( sectors <= 16375 ) {
      return 4;
   }
   else if ( sectors <= 32711 ) {
      return 8;
   }
   else {
      return 0xffff;
...
unsigned long fat_calc_hidden_sectors( struct part_long *p )
{
   ...
   while ( q ) {
      if ( q->os_id == 0x0C00 || q->os_id == 0x0E00 || q->os_id == 0x0F00 ||
           q->os_id == 0x1C00 || q->os_id == 0x1E00 || q->os_id == 0x1F00 ) {
...
static unsigned long fat_root_sector_count( struct boot_ms_dos *b )
{
   ...
   return ( (unsigned long)b->root_entr * sizeof( struct dirent ) +
            b->sect_size - 1 ) /
          b->sect_size;
}
...
{
   return b->res_sects + b->num_fats * fat_size( b ) +
          fat_root_sector_count( b );
}

/* calculates number of clusters from filesystem size */
unsigned long fat_cluster_count( struct boot_ms_dos *b )
{
   return ( fat_sector_count( b ) - fat_non_data_sector_count( b ) ) /
          b->clust_size;
}
...
static int fat_type( struct boot_ms_dos *b )
{
   ...
   if ( clusters > FAT16_MAX_DATA_CLUST ) {
      return FAT_32;
   }
   else if ( clusters > FAT12_MAX_DATA_CLUST ) {
      return FAT_16;
   }
   else {
      return FAT_12;
...
static unsigned long fat_max_cluster_entries( struct boot_ms_dos *b,
                                              int fat_type )
{
                         ...
                         unsigned long *value )
{
   ...
   if ( fat_cluster_count( b ) + 2 < cluster ) {
      return FAILED;
   }
...
{
   if ( is_fat32( b ) ) {
      return b->x.f32.label;
   }
   else {
      return b->x.f16.label;
   }
}
...
int fat_update_label_file( struct part_long *p, struct boot_ms_dos *b )
{
   ...
   int fat_typ = fat_type( b );
   if ( ( buf = malloc( SECT_SIZE ) ) == NULL ) {
      return FAILED;
   }

   if ( fat_typ == FAT_32 ) {
      data_cluster = b->x.f32.root_clust;
      if ( fat_get_table_entry( p, b, 0, data_cluster, &next_data_cluster ) ==
           FAILED ) {
         goto failed;
      }
   ...
   while ( 1 ) {
      for ( sect = start_sect; sect < start_sect + sector_count; sect++ ) {
         if ( disk_read_rel( p, sect, buf, 1 ) == FAILED ) {
            goto failed;
         }
         ...
         for ( j = 0; j < DIRENT_PER_SECT; j++ ) {
            if ( dirent->name[0] == 0 ||
                 ( dirent->attr & DIRENT_LONG_NAME_MASK ) !=
                       DIRENT_LONG_NAME_MASK &&
                    ( ( dirent->attr ) &
                      ( DIRENT_ATTR_VOL | DIRENT_ATTR_DIR ) ) ==
                       DIRENT_ATTR_VOL ) {

               if ( memcmp( fat_label( b ), NO_NAME_LABEL,
                            sizeof( b->x.f16.label ) ) ) {
                  /* update label or create new one if not found */
                  memset( dirent, 0, sizeof( struct dirent ) );
                  dirent->attr |= DIRENT_ATTR_VOL;
                  memcpy( dirent->name, fat_label( b ),
                          sizeof( b->x.f16.label ) );
               }
               ...
               if ( disk_write_rel( p, sect, buf, 1 ) == FAILED ) {
                  goto failed;
               }
               goto success;
            }
      ...
      if ( fat_typ == FAT_32 ) {
         if ( fat32_is_end_of_chain( next_data_cluster ) ) {
            break;
         }
         else {
            data_cluster = next_data_cluster;
            if ( fat_get_table_entry( p, b, 0, data_cluster,
                                      &next_data_cluster ) == FAILED ) {
               goto failed;
            }
            start_sect = fat_cluster_to_sector( b, data_cluster );
         }
...
static int fat_calculate_table_size( struct boot_ms_dos *b, int fat_type )
{
   ...
   cluster_bound_high =
      ( fat_sector_count( b ) - b->res_sects ) / b->clust_size;

   if ( fat_type == FAT_32 ) {
      b->x.f32.fat_size =
         FAT32_ENTRY_SIZE * cluster_bound_high / b->sect_size;
      b->fat_size16 = 0;
   }
   else if ( fat_type == FAT_16 ) {
...
static unsigned long fat_calculate_serial( struct part_long *p,
                                           struct boot_ms_dos *b )
{
   return ( ( p->rel_sect << 16 ) + ( p->num_sect * ( (long)b % 451 ) ) ) +
          ( ( dinfo.total_sects % 12345L ) ^
            ( dinfo.total_sects * 67891L ) ) +
                            ...
                            char label[] )
{
   ...
   if ( fat_type == FAT_32 ) {
      ...
      strncpy( b->x.f32.xcode + EMP_IPL_SIZE, MESG_NON_SYSTEM,
               sizeof( b->x.f32.xcode ) - EMP_IPL_SIZE );
   }
   else {
      ...
      strncpy( b->x.f16.xcode + EMP_IPL_SIZE, MESG_NON_SYSTEM,
               sizeof( b->x.f32.xcode ) - EMP_IPL_SIZE );
   }

   if ( cluster_size == 0 ) {
      return FAT_ERR_TOO_SMALL;
   }
   if ( cluster_size == 0xffff ) {
      return FAT_ERR_TOO_LARGE;
   }
                                                    ...
                                                    unsigned long ) )
{
   ...
   for ( copy = 0; copy < b->num_fats; copy++ ) {
      sprintf( buf, "^Format: writing FAT %d of %u ...", copy + 1,
               b->num_fats );
      progress( buf );

      /* fill in the first cluster entries */
      if ( !( fat = malloc( b->sect_size ) ) ) {
         return FAILED;
      }
      ...
      if ( disk_write_rel( p, b->res_sects + fat_sz * copy, fat, 1 ) ==
           FAILED ) {
         free( fat );
         return FAILED;
      }
      ...
      result = part_fill_sectors( p, b->res_sects + fat_sz * copy + 1,
                                  fat_sz - 1, 0, write_progress, NULL );
      if ( result != OK ) {
         return result;
      }
   }
