...
void select_system_type( struct part_long *p )
{
   ...
   write_string( Yellow + BakWhite, FsX + 42, FsY + FsH - 1,
                 " INS - Custom code " );
...
#define BARLEN ( n ) /* (n-2) */
   ...
   while ( 1 ) {
      ...
      if ( N != n ) {
         ...
         for ( i = 0; i < BARLEN; i++ ) {
            write_char( Yellow + BakWhite, BARX, BARY + i,
                        ( i != barpos ) ? '°' : '²' );
         }
      }
...
int two_btn_dialog( char *msg, char *btn1, int res1, char *btn2, int res2 )
{
   ...
   while ( 1 ) {
      ...
      write_string( ( result == res1 ) ? BrWhite + BakBlack
                                       : Black + BakWhite,
                    38 - b1l - 1, y + 2, " " );
      write_string( ( result == res1 ) ? BrWhite + BakBlack
                                       : Black + BakWhite,
      ...
      write_string( ( result == res1 ) ? Black + BakWhite
                                       : BrWhite + BakBlack,
                    41, y + 2, " " );
      write_string( ( result == res1 ) ? Black + BakWhite
                                       : BrWhite + BakBlack,
      ...
      if ( ev.scan == 0x4BE0 || ev.scan == 0x4B00    /* left */
         ...
         if ( ev.x >= 41 && ev.x <= 42 + b2l && ev.y == y + 2 ) {
            if ( ev.left == 1 ) {
               pressed = 1;
            }
         ...
         else if ( ev.x >= 37 - b1l && ev.x <= 38 && ev.y == y + 2 ) {
            if ( ev.left == 1 ) {
               pressed = 1;
            }
            if ( ev.left == 0 && pressed == 1 ) {
...
void boot_menu_options( struct adv *adv )
{
   ...
   write_string( Black + BakWhite, BoX + 2, BoY + 8,
                 MENU_ADV_OPT_IGN_UNUSED );
   ...
   while ( 1 ) {
      ...
      if ( act == 0 ) {
         ...
         edit_int_field( &ev, 0, BrWhite + BakBlack, BoX2, BoY + 5, -3,
                         &timeout, 99 );
      }
      else if ( act == 4 ) {
         write_string( Black + BakWhite, BoX2, BoY + 6, "       " );
         edit_int_field( &ev, 0, BrWhite + BakBlack, BoX2, BoY + 6, -3,
                         &def_menu, MAX_MENU_ROWS );
         if ( def_menu != 0 ) {
         ...
         int ppos = 0;
         password[0] = 0;
         edit_str_field( &ev, 0, BrWhite + BakBlack, BoX2, BoY + 7, 9,
                         password, &ppos );
         if ( ev.key == 13 ) {
...
void edit_boot_menu( struct adv *adv, int p, int flag )
{
   ...
   static char *show[5] = { " ---- ", " Last ", " Next ", " Prev ",
                            " Last3" };
   ...
   write_string( Yellow + BakWhite, BmX + 18, BmY + BmH - 1,
   ...
   while ( 1 ) {
      if ( old_col != col || old_act != act || redraw ) {
         for ( i = 0; i < n; i++ ) {
            ...
            sprintf( tmp, " %2d  %s %2d%s %s %s  %-29s %s ", i + 1,
                     boot_type[adv->menu[i].boot_type], k,
      ...
      if ( col == 0 ) {
         ...
         write_string( BrWhite + BakBlack, BmX + 14, BmY + 3 + act,
      ...
      if ( col == 5 ) {
         edit_str_field( &ev, 0, BrWhite + BakBlack, BmX + 24, BmY + 3 + act,
                         sizeof( adv->menu[act].name ), adv->menu[act].name,
                         &pos );
      }
      else {
         get_event( &ev, EV_KEY );
...
void setup_ipl( struct mbr *mbr )
{
   ...
   while ( 1 ) {
      ...
      if ( act == 1 ) {
         edit_int_field( &ev, 0, BrWhite + BakBlack, SiX + 27, SiY + 2, -3,
                         &n, 64 );
      }
      else {
         get_event( &ev, EV_KEY );
...
void redraw_menu( int view )
{
   ...
   write_string( MENU_COLOR, 44, 20,
                 ( view == VIEW_ADV ) ? MENU_SAVE_ADV : MENU_SAVE_MBR );
   write_string( HINT_COLOR, 42, 21, "L)" );
   write_string( MENU_COLOR, 44, 21,
                 ( view == VIEW_ADV ) ? MENU_LOAD_ADV : MENU_LOAD_MBR );
   ...
   write_string( HINT_COLOR & ( ( view == VIEW_EMBR ) ? 0x77 : 0xFF ), 56, 24,
                 "F5" );
   write_string( MENU_COLOR & ( ( view == VIEW_EMBR ) ? 0x77 : 0xFF ), 58, 24,
                 " - Disk " );
