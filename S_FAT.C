...
int format_fat(struct part_long *p, char **argv, char **msg)
{
    ...
    if (form_type == F_VERIFY) {
        ...
        result = part_fill_sectors(p, fat_non_data_sector_count(b), 
            fat_sector_count(b) - fat_non_data_sector_count(b), 0, 
            write_progress, fat_bbt_track);
        if (result == FAILED) {
...
int setup_fat(struct part_long *p)
{
    ...
    if (syst == 12 && max_clust > FAT12_MAX_DATA_CLUST) max_clust = FAT12_MAX_DATA_CLUST;
    if (syst == 16 && max_clust > FAT16_MAX_DATA_CLUST) max_clust = FAT16_MAX_DATA_CLUST;
    ...
    while (1) {
        ...
        sprintf(tmp, "%-11lu  max %-11lu", 
            (num_sect - fat_non_data_sector_count(b)) / b->clust_size, max_clust);
