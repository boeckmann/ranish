...
{
   return b->res_sects + b->fat_copies * b->fat_size +
          ( clust - 2 ) * b->clust_size;
}

int fat32_read_clust( unsigned char *buf, struct part_long *p,
                      struct boot_fat32 *b, unsigned long clust )
{
   int i;
   ...
   for ( i = 0; i < b->clust_size; i++ ) {
      if ( disk_read_rel( p, sect + i, buf, 1 ) == FAILED ) {
         return FAILED;
      }
      buf += SECT_SIZE;
   }
...
int fat32_write_clust( unsigned char *buf, struct part_long *p,
                       struct boot_fat32 *b, unsigned long clust )
{
   int i;
   unsigned long sect = fat32_clust_to_sect( b, clust );
   ...
   for ( i = 0; i < b->clust_size; i++ ) {
      if ( disk_write_rel( p, sect + i, buf, 1 ) == FAILED ) {
         return FAILED;
      }
      buf += SECT_SIZE;
   }
...
int setup_fat32( struct part_long *p )
{
   ...
   write_string( TEXT_COLOR, StX, StY + 13, "         Number of clusters:" );
   ...
   if ( b->fs_ver_maj > 0 || b->fs_ver_maj == 0 && b->fs_ver_min > 0 ) {
      show_error(
         "Warning: I wasn't tested with this FileSystem Version!!!" );
   }
   ...
   sprintf( tmp, "%-5u %s bytes/clust", b->clust_size,
   ...
   sprintf( tmp, "%lu sectors = %s KiB", max_num_sect,
   ...
   while ( 1 ) {
      ...
      write_string( ( b->num_sects == p->num_sect ) ? DATA_COLOR
                                                    : INVAL_COLOR,
                    StX2, StY + 12, tmp );
      ...
      if ( act == 4 ) {
         edit_int_field( &ev, 0, EDIT_COLOR, StX2, StY + 11, -11,
                         &b->hid_sects, -1 );
      }
      if ( act == 5 ) {
         edit_int_field( &ev, 0, EDIT_COLOR, StX2, StY + 12, -11,
                         &b->num_sects, -1 );
      }
      /*   get_event(&ev,EV_KEY); */
      ...
      else if ( ev.scan == 0x4100 ) /* F7 - Calculate size */
      {
         ...
         min_clust = ( lc + 3 ) / 4 - 2;
         min_num_sect = b->res_sects + b->fat_copies * b->fat_size +
                        min_clust * b->clust_size;

         sprintf( tmp, "%lu sectors = %s KiB", min_num_sect,
