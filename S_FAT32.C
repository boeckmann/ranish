...
/* tmp2=tmp+120; */
 ...
 write_string( TEXT_COLOR, StX, StY+ 1,"                  System id:                             FS Version:");
 ...
 write_string( TEXT_COLOR, StX, StY+ 5,"      First cluster in Root:                               Ext sign:");
 ...
 write_string( TEXT_COLOR, StX, StY+12,"    Total number of sectors:               123456789");
 ...
 sprintf(tmp,"%-5u %s bytes/clust", b->clust_size, sprintf_long(tmp1,(unsigned long)b->clust_size*SECT_SIZE) );
 write_string( DATA_COLOR, StX2, StY+3, tmp );

 sprintf(tmp,"%d", b->sect_size );
 write_string( DATA_COLOR, StX3, StY+3, tmp );

 sprintf(tmp,"%-5lu %s max clust", b->fat_size, sprintf_long(tmp1,max_clust) );
 ...
 while(1)
    {
     ...
     write_string( (b->track_size==dinfo.num_sects)?DATA_COLOR:INVAL_COLOR, StX2, StY+10, tmp );
     ...
     write_string( (b->num_sects==p->num_sect)?DATA_COLOR:INVAL_COLOR, StX2, StY+12, tmp );
     sprintf(tmp,"%08lX", b->magic_num );
     write_string( (b->magic_num==MBR_MAGIC_NUM32)?DATA_COLOR:INVAL_COLOR, StX3-5, StY+6, tmp );
     ...
     if( act==5 )
        edit_int_field(&ev,0,EDIT_COLOR, StX2, StY+12, -9,
                                                &b->num_sects, 999999999L);
/*   get_event(&ev,EV_KEY); */
